...
Usage:
  ...
  bash install-hub.sh --help

Options:
  --doctor    Run diagnostics only. Does not change system state.
  --help      Show this help.

The installer installs the hub, Mosquitto, runtime folders, the md2-hub systemd service,
and then performs guided setup for Zigbee2MQTT, OLA/DMX, and audio when matching hardware is connected.
EOF
}
...
install_service_control_sudoers() {
  local systemctl_bin
  systemctl_bin="$(command -v systemctl || echo /usr/bin/systemctl)"
  local sudoers_file="/etc/sudoers.d/escapehub-systemctl"

  log "Installing limited sudo rule for EscapeHub service control"
  cat >"$sudoers_file" <<EOF
# Allow the EscapeHub web UI to manage only its required services.
# Generated by EscapeHub install-hub.sh.
Cmnd_Alias ESCAPEHUB_SYSTEMCTL = ${systemctl_bin} daemon-reload, \\
  ${systemctl_bin} enable md2-hub.service, \\
  ${systemctl_bin} disable md2-hub.service, \\
  ${systemctl_bin} restart md2-hub.service, \\
  ${systemctl_bin} enable mosquitto.service, \\
  ${systemctl_bin} start mosquitto.service, \\
  ${systemctl_bin} stop mosquitto.service, \\
  ${systemctl_bin} restart mosquitto.service, \\
  ${systemctl_bin} enable zigbee2mqtt.service, \\
  ${systemctl_bin} disable zigbee2mqtt.service, \\
  ${systemctl_bin} start zigbee2mqtt.service, \\
  ${systemctl_bin} stop zigbee2mqtt.service, \\
  ${systemctl_bin} restart zigbee2mqtt.service, \\
  ${systemctl_bin} enable olad.service, \\
  ${systemctl_bin} disable olad.service, \\
  ${systemctl_bin} start olad.service, \\
  ${systemctl_bin} stop olad.service, \\
  ${systemctl_bin} restart olad.service
${RUN_USER} ALL=(root) NOPASSWD: ESCAPEHUB_SYSTEMCTL
EOF
...
install_core() {
  ...
  apt-get update
  apt-get install -y git curl ca-certificates python3 make g++ build-essential mosquitto mosquitto-clients lsof sudo
...
install_zigbee() {
  ...
  if [[ -z "$zigbee_dev" ]]; then
    ...
    echo "Connect the adapter and rerun install-hub.sh to create the stable /dev/zigbee mapping."
...
install_dmx() {
  ...
  if [[ -z "$dmx_dev" ]]; then
    ...
    echo "Connect the adapter and rerun install-hub.sh to create the stable /dev/dmx mapping."
    echo "Detected serial devices:"
    list_serial_devices || true
  else
